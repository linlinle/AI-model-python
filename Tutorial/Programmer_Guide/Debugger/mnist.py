...
debugger wrapper has the same interface as Session, so enabling debugging requires no other changes to the code.
The wrapper provides additional features, including:
*   Bringing up a CLI before and after Session.run() calls, to let you control the execution and
    inspect the graph's internal state.
...
###### error: diff = -(y_ * tf.log(y))
  ...
  #To add support for tfdbg in our example, all that is needed is to add the following lines of code
  #and wrap the Session object with a debugger wrapper.
