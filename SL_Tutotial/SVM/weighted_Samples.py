...
样本权重重新调整了C参数，这意味着分类器更重视正确获取这些点。为了强调这里的效果，我们加入加权异常点，使决策边界的变形非常明显。
'''
import numpy as np
...
def plot_decision_function(classifier, sample_weight, axis, title):
    ...
    axis.contour(xx, yy, Z, alpha=0.75, cmap=plt.cm.bone)
...
# we create 20 points
np.random.seed(0)
X = np.r_[np.random.randn(10,2)+[1,1],np.random.randn(10,2)] #部分样本点右上偏移
y = [1]*10+[-1]*10
