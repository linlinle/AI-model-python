"""展示了使用 AdaBoost-SAMME 的非线性可分两类问题的决策边界和决策函数值"""
...
dbt = AdaBoostClassifier(DecisionTreeClassifier(max_depth=1),algorithm="SAMME",n_estimators=200)
dbt.fit(X,y)
plot_colors = "br"
plot_step = 0.02
class_names = "AB"
...
cd = plt.contourf(xx,yy,Z,cmap=plt.cm.Paired)
plt.axis("tight")
...
twoclass_output = dbt.decision_function(X)
