...
class MtcnnDetector(object):
    ...
    def __init__(self,
        ...
        self.minsize   = float(minsize) # self.minsize最小的人脸尺寸
        self.factor    = float(factor)  # 缩放比
    ...
    def detect_face(self, img):
        ...
        # 将图片按照factor进行缩放，当缩放后的图片的长、宽中，有一个小于等于12时，则停止缩放。
        ...
        for batch in sliced_index:
            iter = zip(repeat(img), self.PNets[:len(batch)], [scales[i] for i in batch], repeat(self.threshold[0]))

            local_boxes = self.Pool.map( detect_first_stage_warpper,iter)
