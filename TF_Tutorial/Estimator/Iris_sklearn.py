...
def main(unused_argv):
  ...
  #实例化相关的Pre-made Estimators
  #DNNClassifier是Pre-made Estimators类，通过密集的前馈神经网络训练分类模型
  classifier = tf.estimator.DNNClassifier(
      feature_columns=feature_columns, hidden_units=[10, 20, 10], n_classes=3,
      config=run_config)

  # Train.
  # Train.用于将特征和目标数据传递给train，evaluate和predict方法
  # 最终返回[['key'=特征名，'value'=特征数据]，真实标签数据]
  # 这里满足numpy格式，也可以用pandas格式pandas_input_fn
